...
const CryptodDetailsDashBoard = () => {
	...
	const [idCheck, setIdCheck] = useState(coin);
	...
	function fetchPrice() {
		fetch(
			...
			.then((data) => setPrice(data[idCheck]));
	}
	...
	return (
		<section className="flex flex-col py-4 pl-2  h-[711px] bg-white rounded-lg max-w-full sm:pl-5">
			<header className="flex gap-6 justify-start whitespace-nowrap items-center sm:flex-wrap sm:pr-5 sm:max-w-full">
				<div className="flex">
					<img
						src={coinsData?.image?.small}
						alt="Bitcoin Icon"
					...
					<div className="flex pl-2 mt-1">
						...
						<div className=" font-medium m-2 text-gray-500 uppercase ">
							{coinsData.symbol}
						</div>
					</div>
				</div>
